...
class Robot(pygame.sprite.Sprite):
    ...
    def update(self, groups: List, hero: List, monstergroup: List, assets, *args: any, **kwargs: any) -> None:
        ...
        if self.move_points == 0:
            ...
            if rnd == 0:
                temp.add(Robot(assets, (self.rect.x-40, self.rect.y),
                         40, self.image, self.type, self.difficulty))
            ...
            if rnd == 2:
                temp.add(Robot(assets, (self.rect.x, self.rect.y-40),
                         40, self.image, self.type, self.difficulty))
        ...
        for x in range(len(self.done_map)):
            for y in range(len(self.done_map[x])):
                if (self.done_map[x][y])[0] == 'melee_e':
                    self.monster = Robot(self.assets,  ((self.done_map[x][y])[1]), 5, pygame.transform.scale(
                        pygame.image.load(assets['melee_e']), self.uni_size), "m", 1)
                    self.monstergroup.add(self.monster)
                if (self.done_map[x][y])[0] == 'big_melee_e':
                    self.monster = Robot(self.assets,  ((self.done_map[x][y])[1]), 7, pygame.transform.scale(
                        pygame.image.load(assets['big_melee_e']), self.uni_size), "m", 2)
                    self.monstergroup.add(self.monster)
                if (self.done_map[x][y])[0] == 'shooting_e':
                    self.monster = Robot(self.assets,  ((self.done_map[x][y])[1]), 10, pygame.transform.scale(
                        pygame.image.load(assets['shooting_e']), self.uni_size), "s", 1)
                    self.monstergroup.add(self.monster)
                if (self.done_map[x][y])[0] == 'big_shooting_e':
                    self.monster = Robot(self.assets,  ((self.done_map[x][y])[1]), 15, pygame.transform.scale(
                        pygame.image.load(assets['big_shooting_e']), self.uni_size), "s", 2)
                    self.monstergroup.add(self.monster)
