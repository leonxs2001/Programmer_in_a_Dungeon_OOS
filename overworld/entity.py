...
class Player(pygame.sprite.Sprite):
    def __init__(self, assets, size, pos):
        ...
        self.image = pygame.transform.scale(
            pygame.image.load(assets['player']), size)
...
class Entity(pygame.sprite.Sprite):
        ...
        for x in range(len(self.done_map)):
            for y in range(len(self.done_map[x])):
                if (self.done_map[x][y])[0] == 'player':
                    self.player = Player(
                        self.assets, self.uni_size, ((self.done_map[x][y])[1]))
    ...
    def move(self, event, groups: List, monstergroup: pygame.sprite.Group):
        ...
        if event.key == pygame.K_LEFT:
            ...
            temp.add(Player(self.assets, self.uni_size,
                     (self.player.rect.x-40, self.player.rect.y)))
        ...
        if event.key == pygame.K_UP:
            ...
            temp.add(Player(self.assets, self.uni_size,
                     (self.player.rect.x, self.player.rect.y-40)))
