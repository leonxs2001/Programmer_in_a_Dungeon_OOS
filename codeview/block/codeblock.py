...
class CodeBlock(Block):
    ...
    def build(self):
        ...
        # create the circle radius from height of the invisible part
        self.circle_radius = (
            size.y - self.visible_size.y) + self.circle_overlap
        self.circle_x = size.x / 2
        circle_position = pygame.Vector2(
            self.circle_x, self.visible_size.y - self.circle_overlap)
        pygame.draw.circle(self.image, self.background_color,
                           circle_position, self.circle_radius)
        pygame.draw.circle(self.image, (0, 0, 0), (self.circle_x,
                           self.visible_size.y - self.circle_overlap), self.circle_radius, width=2)

        # rect for covering the circlelines inside the rect
        rect = pygame.rect.Rect(
            (0, 0), (self.circle_radius * 2, self.circle_radius + self.circle_overlap))
    ...
    def get_last_invisible_rect(self):
        ...
        if self.next_block:  # if this block has one next_block pass it on to it
            ...
            invisible_size = (self.visible_size.x,
                              CodeBlock.invisible_size_y * self.scale_factor)
