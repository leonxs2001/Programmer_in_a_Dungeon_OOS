...
class TwoSidedBlock(CodeBlock):
    ...
    def build(self):
        ...
        pygame.draw.circle(self.image, INVISIBLE_COLOR,
                           (self.circle_x, -self.circle_overlap), self.circle_radius)
        pygame.draw.circle(self.image, (0, 0, 0), (self.circle_x, -
                           self.circle_overlap), self.circle_radius, width=2)
    ...
    def adjust_to_parent(self):
        ...
        if self.parent_block:
            movement = self.parent_block.get_connection_point_bottom(
                self) - self.get_connection_point_top()
            self.move(movement)
