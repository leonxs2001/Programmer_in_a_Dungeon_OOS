        ...
        if sequence[0][0] == ".":  # the block is a method block
            ...
            for count, param in enumerate(parameters):
                if isinstance(param, tuple):  # is a block to
                    block.input_fields[count].append(
                        get_block_from_expression(param))
                    block.rebuild()
                else:  # is only a normal value
                    block.input_fields[count].value = get_string_from_value(
                        param)
            ...
            if len(sequence[0]) == 3:  # is only a if
                ...
                if_false_block = get_blocks_from_sequence(
                    sequence[0][3], block_dict)
                if if_false_block:
            ...
            if_true_block = get_blocks_from_sequence(
                sequence[0][2], block_dict)
            if if_true_block:
            ...
            if isinstance(sequence[0][2], tuple):
                block.input_fields[0].append(
                    get_block_from_expression(sequence[0][2]))
            else:
                block.input_fields[0].value = get_string_from_value(
                    sequence[0][2])
...
def get_block_from_expression(expression):
    ...
    if expression[0] == ".":  # it is a Method with return value
        ...
        block_dict[(name, "variabeldefinition")
                   ] = VariabelDefinitionBlock(name)
