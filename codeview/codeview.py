        ...
        if self.wait_for_input:
            if event.type == KEYDOWN:
                ...
                if pygame.mouse.get_pressed()[0]:
                    result = self.text_input.check_collision(
                    ...
                    if result:
                        if isinstance(result, str):
                            ...
                            self.data_accessor.save_item(
                                result, code, initialization_code, opponent=False)
                            return True
                        else:
                            self.wait_for_input = False
                ...
                if pygame.mouse.get_pressed()[0]:
                    ...
                    if result or result == 0:
                        if str(result).isnumeric():
                            ...
                            code_block = get_blocks_from_string(
                                item[0], block_dict)
                            initialization_code_block = get_blocks_from_string(
                                item[1], block_dict)
                            ...
                            if initialization_code_block:
                                initialization_block.append(
                                    initialization_code_block)
                            start_block.update_scale_factor(self.scale_factor)
                            initialization_block.update_scale_factor(
                                self.scale_factor)
                            self.code_block_list = [
                                start_block, initialization_block]
            ...
            if event.type == MOUSEBUTTONDOWN:
                if pygame.mouse.get_pressed()[0]:
                    ...
                    if self.menu.is_mouse_on_save():
                        ...
                        # give it the names and ids
                        self.selection_input.load(
                            self.data_accessor.get_all_items())
                    else:
                        # check if we got a new Block by colliding
                        selector_collision = self.selector.check_collision(
                            pygame.Vector2(mouse_position))
                        if selector_collision:
                            if isinstance(selector_collision, Block):  # add to the block list
                                ...
                                selector_collision.update_scale_factor(
                                    self.scale_factor)
                                pos = mouse_position - selector_collision.get_size() / 2
                                selector_collision.move(
                                    pos - selector_collision.position)
                            ...
                            # backwards because we want to grab the one we see
                            for code_block in self.code_block_list[::-1]:
                                # get colliding block or a None
                                collider = code_block.get_collider(
                                ...
                                if collider and isinstance(collider, Block):
                                    ...
                                    if collider == code_block:
                                        # remove the element, for putting it later to the end
                                        self.code_block_list.remove(collider)
